...
export class MemStorage implements IStorage {
  ...
  private seedData() {
    ...
    transactions.forEach(transaction => {
      const id = randomUUID();
      this.transactions.set(id, { description: (transaction as any).description ?? null, originalAmount: transaction.originalAmount ?? null, originalCurrency: transaction.originalCurrency ?? null, merchantIcon: (transaction as any).merchantIcon ?? null, iconColor: (transaction as any).iconColor ?? null, ...transaction, id });
    });
