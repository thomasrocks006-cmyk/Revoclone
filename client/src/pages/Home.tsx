...
export default function Home() {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 px-3 pt-3">
          <div className="flex items-center gap-2">
            {/* Avatar + notification dot (exact corner placement) */}
            ...
            <div className="flex-1">
              ...
              >
                ...
                <span className="text-white/75 text-[13px] leading-none">Search</span>
              </button>
            </div>

            {/* Right icons lighter & spaced */}
        ...
        {/* Balance block — more space below search, tighter decimal spacing */}
      ...
      {/* Quick Actions (more space below Accounts) */}
        ...
        <div className="rounded-[22px] bg-[#0F1224]/95 backdrop-blur-md p-4 shadow-[0_12px_28px_rgba(0,0,0,0.45)]">
          ...
          {!transactionsLoading && !txError && recentTransactions.length > 0 && (
            recentTransactions.map((t, idx) => {
              const name = "merchant" in t ? ((t as any).merchant ?? "Payment") : "Payment";
              const sub = [
              ...
              return (
                <div key={idx} className={`flex items-center ${idx === 0 ? "" : "mt-6"}`}>
                  ...
                  <div className="text-[15px] tabular-nums text-right min-w-[76px] pr-2">
                    {negative ? (
                      <span className="text-[#ff6b6b]">-{AUD.format(Math.abs(amountNum)).replace("A$", "$")}</span>
                    ) : (
                      <span className="text-white/95">{AUD.format(amountNum).replace("A$", "$")}</span>
                    )}
                  </div>
                </div>
              );
          ...
          {!transactionsLoading && !txError && recentTransactions.length === 0 && (
            <>
              <div className="flex items-center">
                ...
                <div className="text-[15px] tabular-nums text-right min-w-[76px] pr-2">
                  <span className="text-white/95">{AUD.format(1.55).replace("A$", "$")}</span>
                </div>
              </div>
              ...
              <div className="flex items-center mt-6">
                ...
                <div className="text-[15px] tabular-nums text-right min-w-[76px] pr-2">
                  <span className="text-[#ff6b6b]">-{AUD.format(18).replace("A$", "$")}</span>
                </div>
              </div>
            </>
          )}
      ...
      <section className="px-4 mt-2">
        <div className="rounded-[22px] bg-[#0F1224]/95 backdrop-blur-md p-4">
          <div className="text-[34px] font-semibold tabular-nums">{AUD.format(4.29).replace("A$", "$")}</div>
        ...
        <div className="rounded-[22px] bg-[#0F1224]/95 backdrop-blur-md p-4">
          ...
          <div className="mt-3 h-[88px] relative">
            <svg viewBox="0 0 300 80" className="absolute inset-0 w-full h-full" aria-hidden="true">
              ...
              {/* Shape closer to the screenshot (flat then step) */}
      ...
      <section className="px-4 mt-2 mb-24">
        <div className="rounded-[22px] bg-[#0F1224]/95 backdrop-blur-md p-4">
          <WatchRow dotClass="bg-[#EADFC7]" label="Gold" sub="XAU to AUD" price="$5,135.7332" pct="▼ 0.07%" />
          <div className="h-4" />
          <WatchRow dotClass="bg-[#1E4FFF]" label="Euro" sub="EUR to AUD" price="$1.7950" pct="▼ 0.18%" flag />
...
/* --- tiny sanity checks (console only) --- */
if (typeof window !== "undefined") {
  // Ensure newline demo string is properly terminated when used
